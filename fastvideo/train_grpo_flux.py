...
def prepare_optimization_data(
    ...
    print(pooled_prompt_embeds)
    print(encoder_hidden_states.shape)
    print("all_log_probs", all_log_probs.shape)
    #exit()
    samples = {
...
def _compute_and_backward_loss(args, sample, perms, i, t_idx, transformer, sigma_schedule, train_timesteps):
    """
    ...
    for k in range(max_len):
        ...
        if k < len(samples_batched_list):
            ...
            for t_idx in range(train_timesteps):
                loss, ratio = _compute_and_backward_loss(args, sample1, perms, k, t_idx, transformer, sigma_schedule, train_timesteps)
        ...
        if dist.get_rank() % 8 == 0:
            if k < len(samples_batched_list):
                print(f"[Set 1, Batch {k}] reward: {sample1['rewards'].item():.4f}, advantage: {sample1['advantages'].item():.4f}, ratio: {ratio.mean().item():.4f}, loss: {loss.item():.4f}")
            else:
                print(f"[Set 2, Batch {k}] reward: {sample2['rewards'].item():.4f}, advantage: {sample2['advantages'].item():.4f}, ratio: {ratio_2.mean().item():.4f}, loss: {loss_2.item():.4f}")
