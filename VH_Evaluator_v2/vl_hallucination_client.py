#!/usr/bin/env python3
"""
VH_Evaluator_v2: Visual Hallucination Evaluator with Image Preprocessing for Grid Detection

This version applies image preprocessing methods (Laplacian, CLAHE, Bilateral)
specifically to enhance grid pattern detection by the MLLM evaluator.

Key difference from v1:
- Preprocessing is ONLY used for grid pattern detection
- Other metrics (detail_sharpness, irrelevant_details) are evaluated on the original image
- The preprocessed images help reveal subtle grid patterns not visible in the original
"""
...
class VLHallucinationClient:
    """VL Hallucination Evaluation Client with Image Preprocessing."""
    
    # Available preprocessing methods
    PREPROCESSING_METHODS = ['laplacian', 'clahe', 'bilateral']
    
    def __init__(self, api_url, preprocessing_methods=None):
        """
        Initialize the evaluation client.
        
        Args:
            api_url: URL of the VLM API server
            preprocessing_methods: List of preprocessing methods to apply
                                  Options: 'laplacian', 'clahe', 'bilateral'
                                  Default: all three methods
        """
        self.api_url = api_url.rstrip('/')
        self.preprocessing_methods = preprocessing_methods or self.PREPROCESSING_METHODS
        
        print(f"🔗 Connected to API: {self.api_url}")
        print(f"🔧 Preprocessing methods: {', '.join(self.preprocessing_methods)}")
    ...
    def _preprocess_image(self, image_path):
        """
        Apply preprocessing methods to enhance texture visibility.
        
        Args:
            image_path: Path to the image file
        
        Returns:
            Dictionary mapping method names to enhanced images (numpy arrays)
        """
        try:
            enhancer = TextureEnhancer(image_path)
            preprocessed = {'original': enhancer.original}
            
            if 'laplacian' in self.preprocessing_methods:
                preprocessed['laplacian'] = enhancer.laplacian_enhancement(strength=2.5)
            
            if 'clahe' in self.preprocessing_methods:
                preprocessed['clahe'] = enhancer.clahe_enhancement(clip_limit=6.0, tile_size=6)
            
            if 'bilateral' in self.preprocessing_methods:
                preprocessed['bilateral'] = enhancer.bilateral_filter_enhancement(
                    d=7, sigma_color=50, sigma_space=50, strength=3.0
                )
            
            return preprocessed
        except Exception as e:
            print(f"⚠️  Preprocessing failed: {e}")
            # Return only original if preprocessing fails
            img = cv2.imread(image_path)
            return {'original': img} if img is not None else {}
    ...
    def _build_evaluation_prompt(self, include_preprocessing_note=True):
        """Construct the evaluation prompt."""
        preprocessing_note = ""
        if include_preprocessing_note:
            preprocessing_note = """
**IMPORTANT: Preprocessing for Grid Pattern Detection**
The images below include both the original image and preprocessed versions using:
- **Laplacian Enhancement**: Enhances edges and fine details
- **CLAHE Enhancement**: Improves local contrast to reveal hidden patterns
- **Bilateral Enhancement**: Preserves edges while enhancing texture details

**NOTE**: The preprocessed images are provided ONLY for grid pattern detection. 
- For `grid_pattern_score`: Examine ALL versions (original + preprocessed) to detect hidden grid patterns
- For `detail_sharpness_score` and `irrelevant_details_score`: Evaluate based on the ORIGINAL image only

"""
        
        prompt = f"""You are a professional image quality assessment expert specializing in evaluating over-optimization hallucination phenomena in images.
...
- **Pay special attention to the preprocessed images** as they may reveal grid patterns not visible in the original
...
{preprocessing_note}## Evaluation Task

Please evaluate the over-optimization hallucination phenomena in the following images.

**CRITICAL Evaluation Rules**:
1. **Grid Pattern Score**: Examine BOTH the original image AND all preprocessed versions. Grid patterns often become visible only after preprocessing.
2. **Detail Sharpness Score**: Evaluate based on the ORIGINAL image ONLY. Do not consider the preprocessed versions.
3. **Irrelevant Details Score**: Evaluate based on the ORIGINAL image ONLY. Do not consider the preprocessed versions.

## Scoring Criteria

- **Over-optimization Level**: 0-5 points, 0 means no problem at all, 5 means extremely severe
  - detail_sharpness_score: Detail sharpening level (edge harshness, unnatural sharpening, high-frequency noise) - **ORIGINAL IMAGE ONLY**
  - irrelevant_details_score: Irrelevant details level (extra details unrelated to prompt, over-decoration, information overload) - **ORIGINAL IMAGE ONLY**
  - grid_pattern_score: Grid texture level - **Check ALL versions including preprocessed images**
...
1. **Grid Pattern Detection** (Use ALL images including preprocessed):
   - Look for regular, repeating patterns in the preprocessed versions
   - Check if Laplacian/CLAHE/Bilateral enhanced images reveal hidden grids
   - Grid patterns may appear as subtle regular textures in smooth areas

2. **Detail Inspection** (ORIGINAL image only):
   - Are there unnecessary detail accumulations?
   - Are edges over-sharpened?
   - Are there elements unrelated to the original intent?

3. **Naturalness Assessment** (ORIGINAL image only): Does the image look natural overall, or does it appear "over-processed"?
...
- When you see **large amounts of irrelevant details** in the ORIGINAL image, increase `irrelevant_details_score`
- When you see **over-sharpened edges** in the ORIGINAL image, increase `detail_sharpness_score`
- When you see **grid patterns in ANY version (original or preprocessed)**, increase `grid_pattern_score`
...
{{
    "image": {{
        ...
        "confidence": 0-1,
        "grid_pattern_detected_in": ["original", "laplacian", "clahe", "bilateral"],
        "detailed_analysis": "Detailed analysis description including which preprocessing method revealed grid patterns"
    }}
}}
```

Please carefully analyze the images and return the evaluation results in the above JSON format.
Remember: Use preprocessed images ONLY for grid pattern detection. Other scores should be based on the original image."""
        return prompt

    def process_image(self, image_path, output_file):
        """Process a single image by calling the remote API with preprocessing."""
        max_retries = 3
        attempt = 0
        result = None
        ...
        while attempt < max_retries:
            try:
                ...
                # Preprocess image using multiple methods
                ...
                # Append target image and all preprocessed versions
                content.append({"type": "text", "text": f"\n## Image to Evaluate: {image_name}\n"})
                
                # Add original image
                content.append({"type": "text", "text": "\n### Original Image:\n"})
                original_b64 = self._encode_image(preprocessed_images['original'])
                content.append({"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{original_b64}"}})
                
                # Add preprocessed versions
                method_descriptions = {
                    'laplacian': 'Laplacian Enhanced (Edge/Detail Enhancement)',
                    'clahe': 'CLAHE Enhanced (Local Contrast Enhancement)',
                    'bilateral': 'Bilateral Enhanced (Edge-Preserving Detail Enhancement)'
                }
                
                for method in self.preprocessing_methods:
                    if method in preprocessed_images:
                        content.append({"type": "text", "text": f"\n### {method_descriptions.get(method, method)}:\n"})
                        method_b64 = self._encode_image(preprocessed_images[method])
                        content.append({"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{method_b64}"}})
                
                content.append({"type": "text", "text": "\n\nPlease evaluate all versions above and provide your assessment.\n"})
...
def generate_summary_report(output_file, folder_name):
    ...
    # Print summary stats
    print(f"\n{'='*60}")
    print(f"📊 EVALUATION SUMMARY REPORT (V2 with Preprocessing)")
    print(f"{'='*60}")
    print(f"\n📈 Overall Statistics:")
...
def process_image_wrapper(args):
    ...
    image_path, api_url, preprocessing_methods = args
    client = VLHallucinationClient(api_url, preprocessing_methods)
    return client.process_image(image_path, None)


def main():
    parser = argparse.ArgumentParser(
        description="VH_Evaluator_v2: Visual Hallucination Evaluator with Image Preprocessing"
    )
    ...
    parser.add_argument("--output_path", default="./hallucination_results_v2.jsonl",
    ...
    parser.add_argument("--preprocessing", nargs='+', 
                       default=['laplacian', 'clahe', 'bilateral'],
                       choices=['laplacian', 'clahe', 'bilateral'],
                       help="Preprocessing methods to apply (default: all three)")
    
    args = parser.parse_args()
    
    print(f"🔍 VH_Evaluator_v2: Visual Hallucination Evaluator with Image Preprocessing")
    print(f"API URL: {args.api_url}")
    print(f"Folder: {args.folder}")
    print(f"Output: {args.output_path}")
    print(f"Max workers: {args.max_workers}")
    print(f"Preprocessing methods: {', '.join(args.preprocessing)}")
    if args.limit:
        print(f"Limit: {args.limit} images")
    
    # Initialize client for connection test
    print(f"\n🔗 Initializing client...")
    test_client = VLHallucinationClient(args.api_url, args.preprocessing)
    ...
    print(f"🔧 Each image will be preprocessed with: {', '.join(args.preprocessing)}")
        ...
        for image_path in images:
            futures.append(
                executor.submit(
                    ...
                    (image_path, args.api_url, args.preprocessing)
                )
            )
